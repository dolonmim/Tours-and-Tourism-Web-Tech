...
//payment time login
	...
	function search($user){
		...
		$sql = "select * from paymentlist where email like '{$user['SEARCH']}'";
...
function payment($email, $name, $h_m_day, $package, $amount,$file)
	{
		$conn = getConnection();
		...
		{
		$sql = "INSERT INTO paymentlist (email, name, h_m_day, package, amount,file) VALUES ('{$email}','{$name}','{$h_m_day}','{$package}','{$amount}','{$file}')";
		echo $sql;
		if(mysqli_num_rows($matching))
		{
		...
		else
		{
			if(mysqli_query($conn, $sql))
			{
			move_uploaded_file($_FILES["file"]["temp_name"],"upload/".$_FILES["file"]["name"]);
